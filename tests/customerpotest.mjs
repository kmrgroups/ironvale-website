...
/* Month keys from the LOCAL calendar, never toISOString(). The three helpers
   here used to mix local date arithmetic with UTC serialisation, so in a
   timezone ahead of UTC (this project's works runs on Asia/Kolkata, +05:30)
   the 1st of a month at 02:00 local is still the previous month in UTC —
   nextMonth came out equal to thisMonth and two checks failed every night
   between midnight and 05:30. The app's own addMonths() already does this
   correctly; these now match it. */
const monthKeyOf = d => d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0');
const monthFromNow = n => { const d = new Date(); d.setDate(1); d.setMonth(d.getMonth() + n); return monthKeyOf(d); };
const thisMonth = monthFromNow(0);
...
const nextMonth = monthFromNow(1);
const monthAfter = monthFromNow(2);
