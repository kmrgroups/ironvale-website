...
(function (root) {
  ...
  /* `tier` is which of the three headings the sign-in screen was on —
     'developer', 'admin' or 'user'. The server checks it only after the
     password, and only to say "that is not a User login" rather than dropping
     somebody onto a menu they did not expect. It is not a security boundary:
     every screen is gated on the role in the record. */
  async function signIn(user, pass, tier) {
    const j = await api('/api/auth', {
      method: 'POST',
      body: JSON.stringify({ action: 'login', user: user, pass: pass, tier: tier || undefined })
    });
    if (j.token) setToken(j.token);
    return j;                      // { token, user, role } or { needCode:true, user, sentTo }
  }
  /* Creating your own login. It comes back inactive — the developer
     administrator approves it — so nothing is signed in here and no token is
     set, deliberately. */
  ...
  /* Changing a password without being signed in: the old password is the
     credential, which is why this works from the sign-in screen. A session is
     started on success, exactly as signing in would. */
  async function changePassword(user, oldPass, newPass) {
    const j = await api('/api/auth', {
      method: 'POST',
      body: JSON.stringify({ action: 'change', user: user, oldPass: oldPass, newPass: newPass })
    });
    if (j.token) setToken(j.token);
    return j;
  }
  ...
  const idms = {
    ...
    /* ---------------- the bell: personal, never shared ---------------- */
    notif: {
      list: () => api('/api/idms?what=notif'),
      push: (toUsers, type, title, bodyText, link) => api('/api/idms', {
        method: 'POST', body: JSON.stringify({ what: 'notif',
          toUsers: Array.isArray(toUsers) ? toUsers : [toUsers],
          type: type, title: title, body: bodyText || '', link: link || '' })
      }),
      /* Every active user whose menu currently includes that department's
         screens — the same rule isPermitted() uses for the menu itself, run
         server-side because working out who qualifies means reading every
         user's permissions, and that list is not something a non-admin
         caller (the one moving an enquiry to NPD or PPC & MMD) is allowed to
         fetch. See DEPT_SCREENS in server/routes/idms.js for what counts as
         each department's screens. */
      pushDept: (dept, type, title, bodyText, link) => api('/api/idms', {
        method: 'POST', body: JSON.stringify({ what: 'notif', toDept: dept,
          type: type, title: title, body: bodyText || '', link: link || '' })
      }),
      markRead: ids => api('/api/idms', {
        method: 'PATCH', body: JSON.stringify({ what: 'notif', ids: Array.isArray(ids) ? ids : [ids] })
      }),
      markAllRead: () => api('/api/idms', {
        method: 'PATCH', body: JSON.stringify({ what: 'notif', markAllRead: true })
      })
    },

    /* Active logins, for "assign to" / "message" pickers. */
    directory: () => api('/api/idms?what=directory').then(j => j.users || []),

    /* ---------------- chat: a thread is only ever seen by its own participants ---------------- */
    chat: {
      threads: () => api('/api/idms?what=chat').then(j => j.threads || []),
      thread: threadId => api('/api/idms?what=chat&threadId=' + encodeURIComponent(threadId)),
      start: (participants, subject, refType, refId) => api('/api/idms', {
        method: 'POST', body: JSON.stringify({ what: 'chat',
          newThread: { participants: participants, subject: subject || '',
            refType: refType || '', refId: refId || '' } })
      }).then(j => j.threadId),
      send: (threadId, text) => api('/api/idms', {
        method: 'POST', body: JSON.stringify({ what: 'chat', threadId: threadId, text: text })
      })
    }
  };
  ...
  /* The progress strip and the scroll arrows live in chrome.js, which BOTH
     idms.html and index.html load. They were in this file first, and that was
     wrong: index.html does not load core.js at all — it is a standalone page
     with its own AI gateway — so half the platform would have got neither.
     A small file both pages can load is the only shape that reaches both.

     Guarded, because core.js must keep working if chrome.js is missing: a
     progress strip is worth having, and it is not worth a broken IDMS. */
  function aiStart(label) {
    try { return window.PageChrome && window.PageChrome.aiBusy.start(label); }
    catch (e) { return null; }
  }
  function aiEnd(job) {
    try { if (job && window.PageChrome) window.PageChrome.aiBusy.end(job); } catch (e) {}
  }
  ...
  async function callAI(prompt, opts) {
    opts = opts || {};
    /* opts.label names the job on the strip. Nothing has to pass one — a call
       that does not say what it is doing still shows that something is — but
       the long ones say, because "reading the drawing" and "planning the route"
       wait very differently and only the caller knows which this is. */
    const job = aiStart(opts.label);
    try { return await callAIInner(prompt, opts); }
    finally { aiEnd(job); }
  }
  async function callAIInner(prompt, opts) {
    let res;
    try {
...
/* Reads JSON out of an AI reply. Models break it in five ways that all turn up
  ...
  function parseAiJson(text){
    ...
    for (let i = 0; i < t.length; i++) {
      ...
      if (inStr) {
        ...
        /* The end of the string — or a quote sitting INSIDE it.

           This is the failure a live costing kept hitting: "Expected ',' or
           '}' after property value". A drawing is full of inch marks and
           quoted callouts, so the model writes

             "stockSize": "Bar 1/2" dia x 3000 mm"
             "spec": "M34 x 0.75 "H" class"

           and the string ends early at the inch mark, leaving the rest of the
           text as garbage where JSON expects a comma. It is not a truncated
           reply and it is not the model ignoring the format — it is one
           character that needed escaping, several thousand characters in,
           which is why it was invisible from the screen.

           A real closing quote is only ever followed by , } ] : or the end of
           the reply. Anything else and the quote is part of the text, so it
           is escaped and the string carries on. */
        if (c === quote) {
          var k = i + 1;
          while (k < t.length && /\s/.test(t[k])) k++;
          var nx = t[k];
          if (k >= t.length || nx === ',' || nx === '}' || nx === ']' || nx === ':') {
            inStr = false; out += '"'; continue;
          }
          /* "a":"x" "b":"y" — a missing comma between two members, which would
             otherwise be read as one quote inside a very long string and would
             swallow the rest of the object. */
          if ((nx === '"' || nx === "'") && /^["'][^"']*["']\s*:/.test(t.slice(k))) {
            inStr = false; out += '",'; continue;
          }
          out += '\\"'; continue;
        }
      ...
      /* A value that is not JSON, outside a string. Models produce these
         constantly on a schema with forty numeric fields, and every one of
         them used to be handed straight to JSON.parse:
           "unitCost": NaN            a number it could not work out
           "life": Infinity           a division it did not guard
           "note": undefined          a field it had nothing for
           { grade: "EN24" }          a JavaScript object literal, not JSON
           "confidence": High         a bare word where a string was asked for
           "cutLength": 120 mm        a number with its unit written after it
           "rate": 142/kg             the same, with a slash
           "material": High Carbon Steel      several bare words in a row

         The last three are what a live costing actually failed on, with
         "Expected ',' or '}' after property value in JSON at position 5669" —
         the parser read 120 as the value and then found "mm" sitting after it.
         An earlier fix here made that worse rather than better: the guard that
         protects exponents (1e3) skipped this branch for ANY letter following a
         number, so a unit was passed through untouched.

         So the whole run is taken together, up to the next character that could
         only be structure, and emitted as ONE value. true/false/null pass
         through; NaN/Infinity/undefined become null, which every caller already
         handles as "no value"; a clean number stays a number; anything else
         becomes the string it was plainly meant to be. Quoting each word on its
         own — which is what the previous version did — turned "High Carbon
         Steel" into "High" "Carbon" "Steel", three strings in a row, and that
         is not JSON either. */
      var expo = /[0-9.]$/.test(out) && (c === 'e' || c === 'E') &&
                 /^[eE][+-]?[0-9]/.test(t.slice(i));
      if (!expo && /[A-Za-z_$%]/.test(c)) {
        /* a number already written immediately before this belongs to the same
           value: "120 mm" is one thing, not 120 followed by a mystery */
        var lead = out.match(/(-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?[^,{}\[\]:"']*)$/);
        var pre = '';
        if (lead && /[0-9]/.test(lead[1])) { pre = lead[1]; out = out.slice(0, out.length - pre.length); }
        var w = pre, j = i;
        while (j < t.length && !/[,{}\[\]:"'\n]/.test(t[j])) { w += t[j]; j++; }
        i = j - 1;
        w = w.trim();
        if (w === 'true' || w === 'false' || w === 'null') out += w;
        else if (w === 'NaN' || w === 'Infinity' || w === 'undefined' || w === '-Infinity') {
          /* a leading minus already written for -Infinity would leave -null */
          if (/-\s*$/.test(out)) out = out.replace(/-\s*$/, '');
          out += 'null';
        }
        else if (/^-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?$/.test(w)) out += w;
        else if (w === '') { /* nothing but spaces — drop it */ }
        else out += '"' + w.replace(/\\/g, '\\\\').replace(/"/g, '\\"') + '"';
        continue;
      }
  ...
  root.Core = {
    ...
    api: api, signIn: signIn, signUp: signUp, changePassword: changePassword,
    verifyCode: verifyCode, setToken: setToken, getToken: getToken,
    ...
    /* For work that is not a single callAI but still keeps someone waiting —
       a drawing being converted, a balloon overlay being drawn, a run of
       several calls that should read as one job. start() returns the handle
       end() takes back; always end it in a finally, or the strip stays up.
       It is chrome.js doing the work; this is here so an IDMS screen need not
       know that, and so it is still safe to call if chrome.js did not load. */
    aiBusy: { start: aiStart, end: aiEnd, count: function(){
      try { return window.PageChrome ? window.PageChrome.aiBusy.count() : 0; }
      catch (e) { return 0; } } },
