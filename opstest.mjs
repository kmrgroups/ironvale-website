...
check('a single developer login is called out',
  /one developer login/i.test(txt($('us-list'))), txt($('us-list')).slice(-160));
...
check('the only developer login cannot be demoted',
  /only developer login/i.test(txt($('us-msg'))), txt($('us-msg')));
