...
export default async function handler(req, res) {
  ...
  try {
    ...
    /* ---------- forgotten password: code by email ---------- */
    // Anyone with an email on their account can reset it themselves, so a lost
    // password no longer depends on remembering a recovery code.
    if (action === 'forgotRequest') {
      const who = String(body.user || '').trim().toLowerCase();
      if (!who) return res.status(400).json({ ok: false, error: 'Enter your username or email address.' });

      const rows = await sql`SELECT * FROM users WHERE lower(username) = ${who} OR lower(email) = ${who}`;
      const u = rows[0];

      // Never reveal whether an account exists — otherwise this becomes a way
      // to find out who has logins here.
      if (!u || !u.email) {
        return res.status(200).json({
          ok: true,
          note: 'If that account exists and has an email address on it, a code is on its way.'
        });
      }
      ...
      await sql`
        INSERT INTO login_codes (username, code_hash, expires_at, tries)
        VALUES (${u.username}, ${hash('RESET:' + code)}, ${expires}, 0)
        ON CONFLICT (username) DO UPDATE
          SET code_hash = EXCLUDED.code_hash, expires_at = EXCLUDED.expires_at, tries = 0`;
      ...
      const text = `Your password reset code is ${code}\n\n`
        ...
        + `If you did not ask to reset your password, ignore this email and tell your developer.`;

      await sendNotification('custom', {
        to: u.email, whatsapp: u.whatsapp || '',
        subject: 'Password reset code: ' + code, text
      });

      const [local, domain] = String(u.email).split('@');
      const masked = local.slice(0, 2) + '•'.repeat(Math.max(2, local.length - 2)) + '@' + domain;
      return res.status(200).json({ ok: true, user: u.username, sentTo: masked,
        note: 'A 6-digit code has been sent to ' + masked });
    }

    if (action === 'forgotVerify') {
      const who = String(body.user || '').trim().toLowerCase();
      const rows = await sql`SELECT * FROM users WHERE lower(username) = ${who} OR lower(email) = ${who}`;
      const u = rows[0];
      const bad = { ok: false, error: 'That code is not valid, or it has expired. Ask for a new one.' };
      if (!u) return res.status(401).json(bad);

      const c = (await sql`SELECT * FROM login_codes WHERE username = ${u.username}`)[0];
      if (!c) return res.status(401).json(bad);
      if (new Date(c.expires_at) < new Date()) {
        await sql`DELETE FROM login_codes WHERE username = ${u.username}`;
        return res.status(401).json(bad);
      }
      if (c.tries >= 5) {
        await sql`DELETE FROM login_codes WHERE username = ${u.username}`;
        return res.status(401).json({ ok: false, error: 'Too many attempts. Ask for a new code.' });
      }
      if (hash('RESET:' + String(body.code || '').trim()) !== c.code_hash) {
        await sql`UPDATE login_codes SET tries = tries + 1 WHERE username = ${u.username}`;
        return res.status(401).json(bad);
      }
      ...
      const newHash = hash(np);
      await sql`UPDATE users SET pass_hash = ${newHash} WHERE username = ${u.username}`;
      if (u.role === 'developer')
        await sql`UPDATE auth SET pass_hash = ${newHash} WHERE id = 1`;
      await sql`DELETE FROM login_codes WHERE username = ${u.username}`;

      return res.status(200).json({ ok: true, token: newHash, user: u.username, role: u.role });
    }
