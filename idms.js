...
/* Which screen keys count as "NPD access" or "PPC & MMD access", for
   notif's toDept targeting below. Kept in sync by hand with the ['npd',...]
   and ['ppc',...] entries in idms.html's MENU array — those are the only
   other place these screen keys are listed, and neither list changes often
   enough to be worth deriving one from the other at runtime. */
const DEPT_SCREENS = {
  npd: ['dash_npd', 'dash_engineering', 'apqp', 'process_master', 'dim_entry', 'pfd', 'fmea',
    'control_plan', 'cnc', 'ppap', 'machine_process', 'pfmea_master', 'pp_spec_master', 'agent_npd'],
  ppc: ['dash_ppc', 'sales_plan', 'sales_monthly_plan', 'production_plan', 'capacity_plan',
    'machine_loading', 'sheet_rawmat', 'dc', 'sales_invoice', 'sales_dashboard', 'form']
};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (what === 'notif') {
      ...
      if (req.method === 'POST') {
        let toUsers = Array.isArray(body.toUsers) ? body.toUsers
          : (body.toUser ? [body.toUser] : []);
        /* Department targeting: resolved here, not by the caller, because
           working out who has NPD or PPC & MMD access means reading every
           user's role/restrictAccess/permissions, and listUsers (the only
           other place that data is readable) is developer/admin-only —
           exactly the accounts that are NOT the ones moving an enquiry to
           NPD or PPC & MMD day to day. Mirrors isPermitted() in idms.html:
           a developer, or anyone whose menu isn't restricted, is in every
           department's audience; a restricted account is in it only if one
           of that department's own screens is in their permissions. */
        if (!toUsers.length && body.toDept && DEPT_SCREENS[body.toDept]) {
          const screens = DEPT_SCREENS[body.toDept];
          const people = await sql`SELECT username, role, restrict_access, permissions
            FROM users WHERE active = true`;
          toUsers = people.filter(u => {
            if (u.role === 'developer') return true;
            if (!u.restrict_access) return true;
            const perms = Array.isArray(u.permissions) ? u.permissions : [];
            return perms.some(p => screens.indexOf(p) !== -1);
          }).map(u => u.username);
        }
