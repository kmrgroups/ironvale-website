...
/* ---------- the three login tiers ----------

   `developer` and `staff` are the two this system has always had; `admin` is
   the Client Administrator the classification work introduced on the menu and
   could not reach, because saveUser only ever stored the other two.

   MANAGES is the whole of the "nobody can view or control the developer login"
   rule, and it lives here rather than on the screen because a menu is not a
   lock: a client administrator may manage their own people — staff and other
   client administrators — and a developer login is neither listed to them, nor
   editable by them, nor deletable by them, nor a role they can grant. Anything
   that reads or writes the users table goes through these two helpers.

   Deliberately NOT done: `admin` is not added to any other route's role check.
   Flushing the database, deleting a part, deleting an employee and revealing
   the provider keys stay developer-only. This tier manages logins; it is not a
   second developer under another name. */
const MANAGES = { developer: ['developer', 'admin', 'staff'], admin: ['admin', 'staff'] };
const manages = (actorRole, targetRole) =>
  (MANAGES[actorRole] || []).includes(targetRole || 'staff');
/* The tier a sign-in screen calls each of these. The words are the user's own
   ("admin login, Developer Admin login, User Login"), kept in one place so the
   screen and the server cannot drift apart about which is which. */
const TIER_OF = { developer: 'developer', admin: 'admin', staff: 'user' };
const TIER_NAME = { developer: 'Developer Admin', admin: 'Admin', user: 'User' };
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'login') {
      ...
      if (u.active === false)
        /* Two different states that look identical on `active` alone, and the
           person in front of the screen needs to know which one they are in:
           waiting on an approval is something they can chase, a deactivation
           is not. */
        return res.status(403).json({ ok: false, pending: !!u.signup_at,
          error: u.signup_at
            ? 'This login has been created and is waiting for your developer administrator to approve it.'
            : 'This account has been deactivated. Contact your administrator.' });
      /* The tier the sign-in screen was on. It is a convenience and not a
         boundary — the role comes from the record either way, and every screen
         is gated on that — but it is checked only AFTER the password, so it
         can never be used to find out what kind of login a name belongs to. */
      if (body.tier && body.tier !== TIER_OF[u.role])
        return res.status(403).json({ ok: false, wrongTier: TIER_OF[u.role],
          error: 'That is not a ' + (TIER_NAME[body.tier] || String(body.tier)) + ' login. Choose ' +
            (TIER_NAME[TIER_OF[u.role]] || 'the right one') + ' at the top and sign in again.' });
    ...
    /* ---------- self sign-up ----------
       Asked for directly: an ordinary user and a client administrator can
       create their own login, with a password and a Face ID enrolment, from
       the sign-in screen. What they cannot do is give themselves access — the
       account is created INACTIVE and stays that way until a developer
       administrator approves it, which is the "all controlled by the Developer
       Admin login" half of the same request.

       The developer tier is not offered here and is refused if asked for. A
       login that can reveal the provider keys and flush the database is not
       something anybody grants themselves. */
    if (action === 'signup') {
      const uname = String(body.user || '').trim();
      const pass = String(body.pass || '');
      if (body.tier === 'developer' || body.role === 'developer')
        return res.status(403).json({ ok: false,
          error: 'A Developer Admin login cannot be created here. Ask your developer administrator.' });
      if (uname.length < 3)
        return res.status(400).json({ ok: false, error: 'Choose a username of at least 3 characters.' });
      if (pass.length < 8)
        return res.status(400).json({ ok: false, error: 'Choose a password of at least 8 characters.' });
      const taken = (await sql`SELECT username FROM users WHERE username = ${uname}`)[0];
      if (taken)
        return res.status(409).json({ ok: false, error: 'That username is already in use. Choose another.' });
      const wanted = body.tier === 'admin' ? 'admin' : 'staff';
      /* The face reading is attached to an account that cannot sign in yet, so
         enrolling at sign-up costs nothing and saves the new person a second
         trip through a screen they have never seen. */
      const face = (Array.isArray(body.descriptor) && body.descriptor.length >= 32)
        ? JSON.stringify(body.descriptor) : null;
      await sql`INSERT INTO users (username, pass_hash, role, email, whatsapp, active,
                  signup_at, signup_note, face_descriptor, face_enrolled_at,
                  pass_changed_at, pass_changed_by)
                VALUES (${uname}, ${scryptHash(pass, newSalt())}, ${wanted},
                  ${String(body.email || '').trim()},
                  ${String(body.whatsapp || '').replace(/\D/g, '')},
                  false, now(), ${String(body.note || '').slice(0, 300)},
                  ${face}::jsonb, ${face ? new Date().toISOString() : null},
                  now(), 'self sign-up')`;
      return res.status(200).json({ ok: true, pending: true, user: uname, face: !!face,
        tier: TIER_OF[wanted],
        note: 'Your login has been created. It is waiting for your developer administrator ' +
              'to approve it before you can sign in.' });
    }
    ...
    if (action === 'listUsers') {
      const actor = await checkRole(token, ['developer', 'admin']);
      if (!actor)
      ...
      const rows = await sql`SELECT username, role, email, whatsapp, twofa, active, auth_methods,
        ...
        face_enrolled_at, pass_changed_at, pass_changed_by, signup_at, signup_note
        FROM users ORDER BY role, username`;
      /* A client administrator is not shown a developer login at all — not
         greyed out, not listed as restricted, simply absent. Filtered here
         rather than on the screen, because a list filtered in the browser has
         already been sent to it. */
      return res.status(200).json({ ok: true, asRole: actor.role,
        manages: MANAGES[actor.role] || [],
        users: rows.filter(r => manages(actor.role, r.role)) });
    }

    if (action === 'saveUser') {
      const admin = await checkRole(token, ['developer', 'admin']);
      if (!admin)
        return res.status(403).json({ ok: false, error: 'Only an administrator can manage users.' });
      const uname = String(body.username || '').trim();
      if (!uname) return res.status(400).json({ ok: false, error: 'Username required.' });
      const allowed = MANAGES[admin.role] || [];
      /* Two separate refusals, because they are two separate mistakes: a
         client administrator may not touch a login above their own tier, and
         may not hand one out either. */
      if (body.role && !allowed.includes(body.role))
        return res.status(403).json({ ok: false,
          error: 'This account cannot give a login the ' +
            (TIER_NAME[TIER_OF[body.role]] || String(body.role)) + ' role.' });
      const role = allowed.includes(body.role) ? body.role : 'staff';
      ...
      if (existing && !manages(admin.role, existing.role))
        return res.status(403).json({ ok: false,
          error: 'That login cannot be changed from this account.' });
      /* The last developer login is the only way back in to manage logins at
         all, and to the flush, the keys and the deletion paths. Demoting it
         locks everybody out with no way to undo it. This was a rule on the
         screen only; a rule that lives on a screen is not a rule. */
      if (existing && existing.role === 'developer' && role !== 'developer') {
        const devs = await sql`SELECT username FROM users WHERE role = 'developer'`;
        if (devs.length <= 1)
          return res.status(400).json({ ok: false,
            error: 'This is the only Developer Admin login. Create a second one before changing this one, ' +
                   'or nobody will be able to manage logins, keys or backups.' });
      }
      if (existing) {
    ...
    if (action === 'deleteUser') {
      const me = await checkRole(token, ['developer', 'admin']);
      ...
      const target = (await sql`SELECT username, role FROM users WHERE username = ${uname}`)[0];
      if (target && !manages(me.role, target.role))
        return res.status(403).json({ ok: false,
          error: 'That login cannot be removed from this account.' });
      if (target && target.role === 'developer') {
        const devs = await sql`SELECT username FROM users WHERE role = 'developer'`;
        if (devs.length <= 1)
          return res.status(400).json({ ok: false,
            error: 'This is the only Developer Admin login. Create a second one before removing this one.' });
      }
