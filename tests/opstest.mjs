...
/* The screen used to say "administrator" for the role stored as 'developer'.
   With three tiers on the sign-in screen — User, Admin and Developer Admin —
   that word is now ambiguous: an "Admin" is a real, different role that cannot
   manage logins above itself. The warning names the tier it actually means. */
check('a single Developer Admin login is called out by name, not as "an administrator"',
  /one Developer Admin login/i.test(txt($('us-list'))), txt($('us-list')).slice(-160));
