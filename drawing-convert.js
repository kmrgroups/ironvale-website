/* ============================================================================
   drawing-convert.js — turn whatever the customer sends into a PNG the
   ...
   before the file is ever uploaded:
     ...
     PDF                    → each page rendered to a PNG; the sender picks
                              the page the drawing is on (page 1 by default)
     PNG / WEBP / GIF / BMP → re-encoded as PNG on a white background
     PNG                   → passed through, downscaled only if very large
     DXF                    → the geometry is drawn to a canvas and saved as a
                              PNG, so a CAD export can still be read
...
(function () {
  ...
  var MAX_EDGE = 3600;        // enough for dimension text; well under the 3MB cap
  var QUALITY = 0.94;
  ...
  function jpegName(name) { return String(name || 'drawing').replace(/\.[^.]+$/, '') + '.png'; }

  /* A drawing on a transparent background becomes black-on-black in PNG, so
     the canvas is filled white first. This was a real failure with exported PNGs. */
  function canvasToDrawingImage(canvas) {
    /* PNG is deliberately preferred for engineering drawings: tiny decimal
       points, plus/minus signs and stacked deviations are line art, not photos.
       PNG ringing around those strokes was a real source of OCR/vision errors. */
    ...
    return readAsDataUrl(file).then(function (url) {
      return new Promise(function (ok, no) {
        ...
        img.onload = function () {
          ...
          ok({
            dataUrl: canvasToDrawingImage(c), pages: 1, analysis:null,
            note: /^image\/jpeg$/i.test(file.type) && d.scale === 1
              ? 'Sent as it is.'
              : 'Converted to PNG' + (d.scale < 1 ? ' and resized to ' + Math.round(d.w) + 'px wide' : '') + '.'
          });
        };
    ...
    return new Promise(function (ok, no) {
      ...
      s.onerror = function () {
        no(new Error('The PDF converter could not be downloaded — you may be offline, or the ' +
          'network may be blocking it. Save the drawing as a PNG and attach that instead.'));
      };
      document.head.appendChild(s);
    });
  }
  /* Preserve PDF text-layer coordinates when the source PDF is vector.
     This is stronger evidence than OCR because the CAD/PDF exporter already knows
     the exact printed text position. Scanned PDFs simply return an empty list and
     fall through to the raster/OCR path. */
  async function pdfTextItems(pg, viewport, pdfPageHeight) {
    if (!pg || !pg.getTextContent) return [];
    try {
      var tc = await pg.getTextContent({ disableCombineTextItems:false });
      return (tc.items||[]).map(function(it){
        var tr=it.transform||[], xPdf=Number(tr[4]||0), yPdf=Number(tr[5]||0);
        var x=Number(viewport.scale||1)*xPdf;
        var y=Number(viewport.height||0)-Number(viewport.scale||1)*yPdf;
        var w=Math.max(1,Number(it.width||0)*Number(viewport.scale||1));
        var fs=Math.max(4,Math.abs(Number(tr[3]||tr[0]||8))*Number(viewport.scale||1));
        return { text:String(it.str||''), x:x, y:Math.max(0,y-fs), w:w, h:fs, dir:it.dir||'ltr' };
    ...
    return loadPdfLib().then(function (lib) {
      return readAsDataUrl(file).then(function (url) {
        ...
        return pdf.getPage(n).then(function (pg) {
          ...
          var textPromise = pdfTextItems(pg, vp, v1.height);
          return pg.render({ canvasContext: c.getContext('2d'), viewport: vp, background: '#FFFFFF' })
            .promise.then(function () {
              return textPromise.then(function(textItems){
                return {
                  dataUrl: canvasToDrawingImage(c), pages: pdf.numPages, page: n,
                  analysis:{ page:n, viewport:{width:vp.width,height:vp.height}, textItems:textItems },
                  note: 'Page ' + n + ' of ' + pdf.numPages + ' converted to PNG' +
  ...
  function dxfToJpeg(file) {
    return readAsText(file).then(function (text) {
      ...
      return {
        dataUrl: canvasToDrawingImage(c), pages: 1, analysis:null,
        note: 'DXF geometry drawn and converted to PNG. Blocks, hatches and dimension ' +
  ...
  function prepare(file, opts) {
    ...
    if (NATIVE_CAD_RE.test(name)) {
      return Promise.reject(new Error(
        ...
        'PDF, DXF or PNG from your CAD software and attach that. Your file can still be sent ' +
        'with the enquiry, but it will not be read automatically.'));
    }
    if (TIFF_RE.test(name)) {
      return Promise.reject(new Error(
        'TIFF drawings cannot be opened by a web browser. Save the drawing as PNG or PDF and attach that.'));
    }
    if (/\.pdf$/i.test(name) || file.type === 'application/pdf') {
      return pdfToJpeg(file, opts.page).then(function (r) {
        return { dataUrl: r.dataUrl, fileName: jpegName(name), note: r.note, pages: r.pages, page: r.page, analysis:r.analysis||null, converted: true };
      });
    }
    if (DXF_RE.test(name)) {
      return dxfToJpeg(file).then(function (r) {
        return { dataUrl: r.dataUrl, fileName: jpegName(name), note: r.note, pages: 1, page: 1, analysis:r.analysis||null, converted: true };
      });
    }
    ...
    return Promise.reject(new Error(
      'That file type cannot be converted here. Attach the drawing as PDF, DXF, PNG or PNG.'));
  }
